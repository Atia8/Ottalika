...
import { CheckCircle, XCircle, Search, Calendar, Clock, AlertCircle, RefreshCw, Building2 } from "lucide-react";
...
export function OwnerPayments() {
  ...
  const [buildings, setBuildings] = useState<Array<{id: number, name: string, apartment_count: number}>>([]);
  ...
  const fetchBuildings = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_URL}/owner/buildings/summary`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  // Filter apartments by search and building
  const filteredApartments = data?.apartments?.filter(apt => {
    const matchesSearch = apt.renter_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         apt.apartment_number?.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesBuilding = selectedBuilding === 'all' || apt.building_id?.toString() === selectedBuilding;
    return matchesSearch && matchesBuilding;
  }) || [];

  // Calculate totals safely with Number conversion
  const totalCollected = filteredApartments
    .filter(a => a.display_status === 'verified')
    .reduce((sum, a) => sum + (Number(a.amount) || Number(a.rent_amount) || 0), 0);
    
  const totalExpected = filteredApartments.reduce((sum, a) => sum + (Number(a.rent_amount) || 0), 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            {data?.buildings?.count === 1 
              ? `Managing ${data.buildings.list[0]}`
              : `Managing ${data?.buildings?.count || 0} buildings`
            }
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {buildings.length > 1 && (
            ...
            >
              ...
              {buildings.map(b => (
                <option key={b.id} value={b.id}>{b.name} ({b.apartment_count} apts)</option>
              ))}
            </select>
          )}
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{data?.summary?.building_count || 0}</p>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                ৳{Number(data?.summary?.total_collected || 0).toLocaleString()}
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Collection Progress - FIXED */}
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
        <h2 className="text-gray-900 font-semibold mb-4">
          Collection Progress for {new Date(monthDate).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}
          {selectedBuilding !== 'all' && buildings.find(b => b.id.toString() === selectedBuilding)?.name && (
            <span className="ml-2 text-sm font-normal text-gray-500">
              - {buildings.find(b => b.id.toString() === selectedBuilding)?.name} only
            </span>
          )}
        </h2>
        <div className="space-y-3">
          <div className="flex justify-between text-gray-600">
            <span>
              Collected: ৳{totalCollected.toLocaleString()} of 
      ...
      {filteredApartments.length > 0 ? (
            ...
            >
                ...
                <div>
                  ...
                  {apt.building_name && (
                    <div className="flex items-center gap-1 mt-1">
                      ...
                      <span className="text-xs text-gray-500">{apt.building_name}</span>
                    </div>
                  )}
                </div>
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-gray-900 font-bold text-lg">৳{Number(apt.rent_amount || 0).toLocaleString()}</span>
                </div>
                  ...
                  <>
                    <div className="border-t border-gray-200 pt-2">
                      <div className="flex justify-between">
                        ...
                        <span className="text-gray-900 font-medium">৳{Number(apt.amount || apt.rent_amount || 0).toLocaleString()}</span>
                      </div>
        ...
        <div className="text-center py-12 bg-gray-50 rounded-xl">
          ...
          <p className="text-gray-500 text-sm mt-1">Try adjusting your search, building filter, or month selection</p>
        </div>
      )}
